...
function factorBoost(manmade = 0) {
  if (game.OP >= V(game.factorBoosts + 3, 1) && game.challenge === 0) {
    if (manmade === 1 && game.fbConfirm === 1) {
      ...
      ${game.upgrades.includes(8) ? " Don't forget to check Incrementy!" : ""}`)) return;
    }
...
function refund() {
  const conf = (game.bConf.ref === 1 ? confirm(`Are you sure you want to refund your booster? 
  You'll reset this Factor Boost!`) : true);
  if (conf) {
...
function collapse(manmade = 0) {
  ...
  function frame() {
    ...
    if (anim === 300) {
      ...
      game.cardinals = game.cardinals.add(EN(Math.max(Math.pow(game.factorBoosts - 24, 2), 0.5)).floor());
