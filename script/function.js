...
function getHalfAlephOmega() {
  const bulk = game.assCard[0].points.min(game.assCard[1].points).min(game.assCard[2].points).divide(2).floor();
...
function calcCard() {
  game.maxCard = 
    EN(Math.max(game.factorBoosts - 24, 2) ** calcCardExponent(game.collapseTime)).floor().max(game.maxCard);
  return game.maxCard;
}
...
function calcFactorShiftTime(n) {
  return Math.max(1 / game.shiftAuto.toNumber(), 100 / calcOPPS(n - 1) * (game.leastBoost <= 1.5 ? 1 : 1) + [200, 729, 4096, 62426, 6 ** 8, 5 ** 11, 4 ** 16, 3 ** 27][n - 1] / (calcTotalMultWOFactor() * ((game.upgrades.includes(1) ? 2 : 1) * (1 + (game.upgrades.includes(11) ? 3 : 0))) ** (n - 1)));
}
...
function calcFactorBoostTime() {
  ...
  return Math.max(1 / game.boostAuto.toNumber(), fbt + BHO * 3 * (getSingLevel() - 1) / (1e270 * calcBupTotalMultWOFactor() * bfact * calcIncrementyMult(game.maxIncrementyRate.pow(0.9)) * (game.aups.includes(4) ? Math.log10(Math.log10(1e280)) : 1))) / getFBmult();
}
...
function increaseOrdTier2(x) {
  const bupCom = x < (BHO / 1e270) ? x * 1e270 : (Math.log10(x / (BHO / 1e270)) / Math.log10(3) + 1) * BHO;
...
function OPtoOrd(x,b) {
  if (x==0) return 0
  let exp=Math.floor(Math.log10(x)+0.000000000001)
  if (validInBase(exp,b)) {
    let coef = Math.floor(x/(10**exp)+0.000000000001)
    if (coef >= b) return b**(OPtoOrd(exp,b)+1)
    return b**OPtoOrd(exp,b)*coef+OPtoOrd(x-coef*10**exp,b)
  } else {
    return b**(OPtoOrd(exp,b))
  }
}

function validInBase(x,b) {
  return x.toString().split("").every(dig => {
